//
//  YXBaseCollectionViewFlowLayout.swift
//  YXSwiftBasicProj
//
...
/** collectionView代理 */
@objc protocol YXBaseCollectionViewDelegate: NSObjectProtocol {
  ...
  /** 每个section 列数（默认2列）*/
  @objc optional func columnNumber(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> Int
  
  /** header高度（默认为0）*/
  @objc optional func referenceSizeForHeader(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> CGSize
  
  /** footer高度（默认为0）*/
  @objc optional func referenceSizeForFooter(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> CGSize
  
  /** 每个section 边距（默认为0）*/
  @objc optional func insetForSection(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> UIEdgeInsets
  
  /** 每个section item上下间距（默认为0）*/
  @objc optional func lineSpacing(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> CGFloat
  
  /** 每个section item左右间距（默认为0）*/
  @objc optional func interitemSpacing(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> CGFloat
  
  /** section头部header与上个section尾部footer间距（默认为0）*/
  @objc optional func spacingWithLastSection(collectionView collection: UICollectionView, layout: YXBaseCollectionViewFlowLayout, section: Int) -> CGFloat
}

class YXBaseCollectionViewFlowLayout: UICollectionViewFlowLayout {

    /** 代理 */
    weak var delegate: YXBaseCollectionViewDelegate?
    ...
    override func prepare() {
        ...
        for idx in 0 ..< sectionCount {
            ...
            let headerAttr = self.layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: indexPath)
            if let header = headerAttr {
            ...
            for item in 0 ..< itemCount {
                ...
                let attr = self.layoutAttributesForItem(at: indexPat)
                if let attr = attr {
